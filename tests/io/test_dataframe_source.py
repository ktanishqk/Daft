"""Tests for DataSource.get_dataframe() unfold (issue #7345)."""
...
class _Unfold(DataSource):
    """Unfold stub: dummy get_tasks raises if the placeholder is ever materialized."""
    ...
    async def get_tasks(self, pushdowns) -> AsyncIterator[DataSourceTask]:
        raise NotImplementedError(
            "unfold source: get_dataframe returned a plan that still contains this DataSource; do not call self.read()"
        )
        yield  # type: ignore[misc]


class Fake(_Unfold):
    """Join two parquet sides. Uses filters only to pick files; does not .where() the residual."""
...
def test_filter_prunes_file_and_still_applies(files: dict[str, str]):
    ...
    # Wrap order: filter then project (Project-first would drop x).
...
def test_explain_limit_count_select(files: dict[str, str]):
    ...
    # #6500: ignore pushdowns.columns inside get_dataframe; join key must survive.
